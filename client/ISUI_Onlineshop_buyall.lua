...
ISBuyallTextBox = ISPanelJoypad:derive("ISBuyallTextBox");

local FONT_HGT_SMALL = getTextManager():getFontHeight(UIFont.Small)
local FONT_HGT_MEDIUM = getTextManager():getFontHeight(UIFont.Medium)

--************************************************************************--
--** ISBuyallTextBox:initialise
--**
--************************************************************************--

function ISBuyallTextBox:initialise()
    ...
    self.yes = ISButton:new((self:getWidth() / 2)  - 5 - buttonWid, self:getHeight() - padBottom - buttonHgt, buttonWid, buttonHgt, getText("UI_Ok"), self, ISBuyallTextBox.onClick);
    ...
    self.no = ISButton:new((self:getWidth() / 2) + 5, self:getHeight() - padBottom - buttonHgt, buttonWid, buttonHgt, getText("UI_Cancel"), self, ISBuyallTextBox.onClick);
    ...
    self.colorBtn = ISButton:new(self.entry.x + self.entry.width + 5, self.entry.y, height, height, "", self, ISBuyallTextBox.onColorPicker);
...
function ISBuyallTextBox:onColorPicker(button)
    self.colorPicker:setX(getMouseX() - 100);
    self.colorPicker:setY(getMouseY() - 20);
    self.colorPicker.pickedFunc = ISBuyallTextBox.onPickedColor;
    self.colorPicker:setVisible(true);
    self.colorPicker:bringToTop();
end

function ISBuyallTextBox:onPickedColor(color, mouseUp)
...
function ISBuyallTextBox:enableColorPicker()
...
function ISBuyallTextBox:setOnlyNumbers(onlyNumbers)
    self.entry:setOnlyNumbers(onlyNumbers);
end

function ISBuyallTextBox:setMultipleLine(multiple)
...
function ISBuyallTextBox:setNumberOfLines(numLines)
    self.numLines = numLines
end

function ISBuyallTextBox:setMaxLines(max)
    self.maxLines = max
...
function ISBuyallTextBox:getMaxLines()
    return self.maxLines
end

function ISBuyallTextBox:setValidateFunction(target, func, arg1, arg2)
	self.validateTarget = target
	self.validateFunc = func
	self.validateArgs = { arg1, arg2 }
end

function ISBuyallTextBox:setValidateTooltipText(text)
	self.validateTooltipText = text
end

function ISBuyallTextBox:destroy()
...
function ISBuyallTextBox:onClick(button)
...
function ISBuyallTextBox:titleBarHeight()
	return 16
end

function ISBuyallTextBox:prerender()
    ...
    if self.entry:getText()~="" then
        self:drawTextCentre(self.text..tostring(self.param1.price*tonumber(self.entry:getText())), self:getWidth() / 2, self.entry:getY() - 8 - fontHgt, 1, 1, 1, 1, UIFont.Small);
...
function ISBuyallTextBox:showErrorMessage(show, errorMsg)
    self.showError = show
    self.errorMsg = errorMsg
end

function ISBuyallTextBox:updateButtons()
...
--************************************************************************--
--** ISBuyallTextBox:render
--**
--************************************************************************--
function ISBuyallTextBox:render()

end

function ISBuyallTextBox:onMouseMove(dx, dy)
    self.mouseOver = true
    if self.moving then
        self:setX(self.x + dx)
        self:setY(self.y + dy)
...
function ISBuyallTextBox:onMouseMoveOutside(dx, dy)
...
function ISBuyallTextBox:onMouseDown(x, y)
...
function ISBuyallTextBox:onMouseUp(x, y)
    if not self:getIsVisible() then
        return;
    end
    self.moving = false
...
function ISBuyallTextBox:onMouseUpOutside(x, y)
    if not self:getIsVisible() then
        return
    end
    self.moving = false
...
function ISBuyallTextBox:onGainJoypadFocus(joypadData)
...
function ISBuyallTextBox:onJoypadDirDown(joypadData)
	self.joypadIndexY = 0
	self.entry:setJoypadFocused(false, joypadData)
end

function ISBuyallTextBox:onJoypadDirUp(joypadData)
	self.joypadIndexY = 1
	self.entry:setJoypadFocused(true, joypadData)
end

function ISBuyallTextBox:onJoypadDown(button, joypadData)
...
--**
--************************************************************************--
function ISBuyallTextBox:new(x, y, width, height, text, defaultEntryText, target, onclick, player, param1, param2, param3, param4)
	local o = {}
...
function ISBuyallTextBox:close()
