...
ISPlayersell = ISPanelJoypad:derive("ISPlayersell");

local FONT_HGT_SMALL = getTextManager():getFontHeight(UIFont.Small)
local FONT_HGT_MEDIUM = getTextManager():getFontHeight(UIFont.Medium)

--************************************************************************--
--** ISPlayersell:initialise
--**
--************************************************************************--

function ISPlayersell:initialise()
    ...
    self.yes = ISButton:new((self:getWidth() / 2)  - 5 - buttonWid, self:getHeight() - padBottom - buttonHgt, buttonWid, buttonHgt, getText("UI_Ok"), self, ISPlayersell.onClick);
    ...
    self.no = ISButton:new((self:getWidth() / 2) + 5, self:getHeight() - padBottom - buttonHgt, buttonWid, buttonHgt, getText("UI_Cancel"), self, ISPlayersell.onClick);
    ...
    self.entry2 = ISTextEntryBox:new(self.defaultEntryText, self:getWidth()*0.07, self:getHeight()*0.6, self:getWidth()*0.37, FONT_HGT_MEDIUM*1.2)
    ...
    self.colorBtn = ISButton:new(self.entry.x + self.entry.width + 5, self.entry.y, height, height, "", self, ISPlayersell.onColorPicker);
...
function ISPlayersell:onColorPicker(button)
    self.colorPicker:setX(getMouseX() - 100);
    self.colorPicker:setY(getMouseY() - 20);
    self.colorPicker.pickedFunc = ISPlayersell.onPickedColor;
    self.colorPicker:setVisible(true);
    self.colorPicker:bringToTop();
end

function ISPlayersell:onPickedColor(color, mouseUp)
...
function ISPlayersell:enableColorPicker()
...
function ISPlayersell:setOnlyNumbers(onlyNumbers)
    self.entry:setOnlyNumbers(onlyNumbers);
end

function ISPlayersell:setMultipleLine(multiple)
...
function ISPlayersell:setNumberOfLines(numLines)
    self.numLines = numLines
end

function ISPlayersell:setMaxLines(max)
    self.maxLines = max
...
function ISPlayersell:getMaxLines()
    return self.maxLines
end

function ISPlayersell:setValidateFunction(target, func, arg1, arg2)
	self.validateTarget = target
	self.validateFunc = func
	self.validateArgs = { arg1, arg2 }
end

function ISPlayersell:setValidateTooltipText(text)
	self.validateTooltipText = text
end

function ISPlayersell:destroy()
...
function ISPlayersell:onClick(button)
...
function ISPlayersell:titleBarHeight()
	return 16
end

function ISPlayersell:prerender()
    ...
    self:drawTextureScaledAspect(ScriptManager.instance:getItem(self.param3):getNormalTexture(), self:getWidth() / 2-20,fontHgt, 40, 40, 1, 1, 1, 1)
...
function ISPlayersell:showErrorMessage(show, errorMsg)
    self.showError = show
    self.errorMsg = errorMsg
end

function ISPlayersell:updateButtons()
...
--**
--************************************************************************--
function ISPlayersell:render()

end

function ISPlayersell:onMouseMove(dx, dy)
    self.mouseOver = true
    if self.moving then
        self:setX(self.x + dx)
        self:setY(self.y + dy)
...
function ISPlayersell:onMouseMoveOutside(dx, dy)
...
function ISPlayersell:onMouseDown(x, y)
...
function ISPlayersell:onMouseUp(x, y)
    if not self:getIsVisible() then
        return;
    end
    self.moving = false
...
function ISPlayersell:onMouseUpOutside(x, y)
    if not self:getIsVisible() then
        return
    end
    self.moving = false
...
function ISPlayersell:onGainJoypadFocus(joypadData)
...
function ISPlayersell:onJoypadDirDown(joypadData)
	self.joypadIndexY = 0
	self.entry:setJoypadFocused(false, joypadData)
end

function ISPlayersell:onJoypadDirUp(joypadData)
	self.joypadIndexY = 1
	self.entry:setJoypadFocused(true, joypadData)
end

function ISPlayersell:onJoypadDown(button, joypadData)
...
--**
--************************************************************************--
function ISPlayersell:new(x, y, width, height, text, defaultEntryText, target, onclick, player, param1, param2, param3, param4)
	local o = {}
